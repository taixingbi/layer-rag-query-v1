...
| `latency` | `{"phase","ms"}` | Once per phase: `embed`, `retrieve_rerank`, `chat`, `follow_up_chat`, `total`. |
...
```
meta
latency(embed) latency(retrieve_rerank)
...
`latency(phase=total)` is **wall-clock** time from the start of the stream handler (right after `meta`) through follow-ups. `embed` is query embedding; `retrieve_rerank` is Qdrant retrieve + chunk rerank combined; `follow_up_chat` is follow-up LLM + rerank combined. `chat` is the sum of every upstream completion in the widen loop plus the final one. Example: `total` ≈ 3.3s with `chat` ≈ 0.9s and `follow_up_chat` ≈ 2.0s is normal when follow-up generation dominates after the main answer.
